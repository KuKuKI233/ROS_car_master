#include "dijistra_planner.h"
#include <pluginlib/class_list_macros.h>

//register this planner as a BaseGlobalPlanner plugin
PLUGINLIB_EXPORT_CLASS(dijistra_planner::DijistraPlanner, nav_core::BaseGlobalPlanner)

namespace dijistra_planner{
    DijistraPlanner::DijistraPlanner(){}
    
    DijistraPlanner::DijistraPlanner(std::string name, costmap_2d::Costmap2DROS* costmap_ros){
        initialize(name, costmap_ros);
    }
    
    void DijistraPlanner::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros){
        if(!initialized_){
            //获取cosmap属性
            ...
            OGM.resize(map_size); //OGM用于存储Astar各点的可检查情况 true/falise；
    ...
    //进行规划
    bool DijistraPlanner::makePlan(const geometry_msgs::PoseStamped& start, 
        ...
        //将世界坐标系wx, wy 转换成地图坐标系 start_x, start_y
        ...
        vector<float> gCosts(map_size, infinity);//gCost起点到node的实际最短路径代价；
        vector<int> cameFrom(map_size, -1);//存储node的父节点（最小代价路径上的）；
        
        //priority_costs为OPEN，即待检查的节点队列；
        //使用multiset保证 priority_costs 是按照 Node.cost有序排列；
        ...
        while(!priority_costs.empty())
        {
            ...
            for(int i = 0; i < neighborIndexes.size(); i++){
                // if(cameFrom[neighborIndexes[i]] == -1)表示 这个neighbor没有检查过，即没有父节点；
    ...
    //计算两节点间的移动成本；
    double DijistraPlanner::getMoveCost(int firstIndex, int secondIndex)
    {
        unsigned int tmp1, tmp2;
    ...
    double DijistraPlanner::getHeuristic(int cell_index, int goal_index)
    {
        unsigned int tmp1, tmp2;
    ...
    bool DijistraPlanner::isInBounds(int x, int y)
    {
    ...
    vector<int> DijistraPlanner::get_neighbors(int current_cell)
    {   
    ...
    void DijistraPlanner::publishPlan(const std::vector<geometry_msgs::PoseStamped>& path) {
    if (!initialized_) {
        ROS_ERROR(
